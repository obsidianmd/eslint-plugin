    ...
    create(context: TSESLint.RuleContext<'settings' | 'general' | 'pluginName' | 'headingEl', []>) {
        ...
        return {
            ...
            ExpressionStatement(node: TSESTree.ExpressionStatement) {
                if (insidePluginSettingTab && node.expression?.type === 'CallExpression') {
                    ...
                    while (
                        ...
                        if (callName === 'setName' && args.length > 0 && args[0]?.type === 'Literal' && typeof args[0].value === 'string') {
                        ...
                        if (callExpr.callee.object && callExpr.callee.object.type === 'CallExpression') {
                    ...
                    if (
                        ...
                        lastCallExpr.callee.object.callee.type === 'Identifier' &&
                        lastCallExpr.callee.object.callee.name === 'Setting' &&
                        methods.includes('setName') &&
                        methods.includes('setHeading')
                    ) {
                        if (typeof text === 'string' && ['settings', 'options', 'configuration', 'config'].some(str => text.toLowerCase().includes(str))) {
                            context.report({
                                node,
                                messageId: 'settings',
                        ...
                        if (typeof text === 'string' && text.toLowerCase().includes('general')) {
                            context.report({
                                node,
                                messageId: 'general',
                                fix: fixer => fixer.remove(node),
                        ...
                        if (typeof text === 'string' && text.toLowerCase().includes(manifest.name.toLowerCase())) {
                            context.report({
                                node,
            ...
            CallExpression(node: TSESTree.CallExpression) {
                if (node.callee.type === 'MemberExpression' && node.callee.property.type === 'Identifier' && node.callee.property.name === 'createEl') {
                    ...
                    if (
                        ...
                        const textProperty = node.arguments[1].properties.find(
                            property =>
                                ...
                                ((property.key.type === 'Literal' && property.key.value === 'text') ||
                                    (property.key.type === 'Identifier' && property.key.name === 'text'))
                        );
                        if (textProperty && textProperty.type === 'Property' && textProperty.value.type === 'Literal' && typeof textProperty.value.value === 'string') {
                    ...
                    if (
                        ...
                        ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(args[0].value)
                    ) {
                        context.report({
                            node: args[0],
                            messageId: 'headingEl',
