...
export default ruleCreator({
    meta: {
        ...
        schema: [],
        messages: {
            missingDefaultPrevented:
                "Check 'evt.defaultPrevented' at the start of '{{eventName}}' handler and return early if already handled.",
            missingPreventDefault:
                "Call 'evt.preventDefault()' in '{{eventName}}' handler after handling the event.",
        },
    },
    defaultOptions: [],
    create(context) {
        ...
        function getWorkspaceOnEventName(node: TSESTree.CallExpression): string | null {
        ...
        function getFunctionBody(node: TSESTree.ArrowFunctionExpression | TSESTree.FunctionExpression): TSESTree.Statement[] | null {
        ...
        function hasDefaultPreventedCheck(statements: TSESTree.Statement[], evtName: string): boolean {
        ...
        function hasPreventDefaultCall(statements: TSESTree.Statement[], evtName: string): boolean {
            const source = context.sourceCode;
            for (const stmt of statements) {
                const text = source.getText(stmt);
